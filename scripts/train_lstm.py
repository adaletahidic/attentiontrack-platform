...
def compute_norm_stats(train_paths: List[Path]) -> Tuple[np.ndarray, np.ndarray, float]:
    """
    NaN-safe mean/std per feature over the training set only.
    Also computes raw pos_weight = n_neg / n_pos.
    """
    sum_ = None
    sumsq = None
    cnt = None
...
@torch.no_grad()
def collect_probs(model_ns, loader, device) -> Tuple[np.ndarray, np.ndarray]:
    model_ns.eval()
    ys = []
    probs = []
    ...
    for X, lengths, y in loader:
        ...
        logits = model_ns(X, lengths).squeeze(-1)
...
def find_best_threshold(
    ...
    """
    Chooses threshold by balanced accuracy by default, not by F1.
    This prevents degenerate 'predict everything positive' solutions.
    """
    best_thr = 0.5
    best_metrics = None
    best_score = -1.0
...
def print_split_stats(name: str, paths: List[Path]) -> None:
    c0 = 0
    c1 = 0
    nan_files = 0
    ...
    total = len(paths)
    pos_ratio = c1 / max(total, 1)
    print(f"{name}: total={total}, y0={c0}, y1={c1}, pos_ratio={pos_ratio:.4f}, nan_files={nan_files}")
...
def main():
    ...
    ap.add_argument("--save_dir", type=str, default="runs/attentiontrack_v3")
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.3,
        help="Scales raw pos_weight = n_neg / n_pos. 0.3-0.5 is a good starting range.",
    )
    ...
    print_split_stats("train", train_paths)
    print_split_stats("val", val_paths)
    print_split_stats("test", test_paths)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        thr, thr_metrics = find_best_threshold(
            y_val_true,
            y_val_prob,
        ...
        print(
            ...
            f"val_spec={val_metrics['spec']:.4f} | "
    ...
    print(f"Saved: {best_path}, {norm_path}, {metrics_path}")
