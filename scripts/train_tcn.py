...
class TCNClassifier(nn.Module):
    def __init__(self, input_size: int, channels=(64, 64, 128), kernel_size=3, dropout=0.3, num_classes=1):
        super().__init__()
    ...
    def forward(self, x, lengths):
        ...
        T = h.size(2)
        mask = (torch.arange(T, device=lengths.device).unsqueeze(0) < lengths.unsqueeze(1)).float()
        mask = mask.unsqueeze(1)

        h = h * mask
        pooled = h.sum(dim=2) / mask.sum(dim=2).clamp(min=1.0)
...
def main():
    ...
    ap.add_argument("--save_dir", type=str, default="runs_full/tcn")
    ...
    model = TCNClassifier(input_size=input_size, channels=(64, 64, 128), kernel_size=3, dropout=args.dropout, num_classes=1).to(device)
